...
def compute_recruitability_score(candidate, extra_signals: dict | None = None) -> dict:
    ...
    return {
        ...
        "details": details,
        "summary": (
            f"Recruitability score {score:.0f}/100. "
            f"{'Open to opportunities. ' if open_to_work >= 0.9 else ''}"
            f"Recruiter response rate: {r_resp*100:.0f}%. "
            f"Interview completion: {interview*100:.0f}%. "
            f"Profile completeness: {completeness*100:.0f}%. "
            f"{'GitHub activity detected.' if github_score > 0 else 'No GitHub activity.'}"
        ),
    }
